package com.example.ui.theme

import androidx.compose.ui.graphics.Color

// Sleek Interface Theme Colors
val SleekBackground = Color(0xFFFEF7FF)
val SleekTextPrimary = Color(0xFF1D1B20)
val SleekTextSecondary = Color(0xFF49454F)
val SleekPrimary = Color(0xFF6750A4)
val SleekOnPrimary = Color(0xFFFFFFFF)
val SleekContainer = Color(0xFFEADDFF)
val SleekOnContainer = Color(0xFF21005D)
val SleekCardBg = Color(0xFFF3EDF7)
val SleekCardBorder = Color(0xFFCAC4D0)
val SleekActiveBg = Color(0xFFE8DEF8)
val SleekActiveBorder = Color(0xFFD0BCFF)
val SleekError = Color(0xFFB3261E)

val Purple80 = Color(0xFFD0BCFF)
val PurpleGrey80 = Color(0xFFCCC2DC)
val Pink80 = Color(0xFFEFB8C8)

val Purple40 = Color(0xFF6650a4)
val PurpleGrey40 = Color(0xFF625b71)
val Pink40 = Color(0xFF7D5260)
